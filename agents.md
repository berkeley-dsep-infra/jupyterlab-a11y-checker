...
The `jupyterlab-a11y-checker` is an accessibility engine for Jupyter Notebooks that helps detect and report accessibility issues aligned with WCAG 2.1 AA guidelines. It can be used as a standalone CLI tool to audit notebooks without requiring JupyterLab to be running.
